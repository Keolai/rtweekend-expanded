...
public:
    ...
    virtual color light_hit(const hittable &world, const ray &r, hit_record &rec, int shadow_samples) const
    {
...
public:
    ...
    color light_hit(const hittable &world, const ray &r, hit_record &rec, int shadow_samples) const override
    {
        hit_record shadow_rec;
        color lighting = color(0.);
        for (int j = 0; j < shadow_samples; j++)
        {
            ...
            if (!world.hit(light_direction, interval(0.001, distance), shadow_rec)) // not blocked
            {
                ...
                lighting +=
                    ...
                    ((distance * distance) * shadow_samples);
            }
        }
