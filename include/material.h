...
public:
  ...
  double min_brightness = 0.0; //this is for glowy textures
...
protected:
  texture mat_texture;
  color normal_default = color(0.5, 0.5, 1.0);
};
...
public:
  ...
  lambertian(const color &albedo, const std::string &normal_path)
  {
    mat_texture = texture(albedo);
...
public:
  ...
  metal(const std::string &file_path, double fuzz) : fuzz(fuzz < 1 ? fuzz : 1)
  {
    mat_texture = texture(file_path);
    normal_texture = texture(normal_default);
  }
  ...
  metal(const color &albedo, const std::string &normal_path, double fuzz) : fuzz(fuzz < 1 ? fuzz : 1)
  {
    mat_texture = texture(albedo);
    normal_texture = texture(normal_path);
  }
